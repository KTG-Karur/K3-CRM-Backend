...
async function createStaffOnDuty(req, res) {
    ...
    try {
        ...
        if (validationResponse != true) {
            ...
            responseEntries.data = await staffOnDutyServices.createStaffOnDuty(req.body);
            if (!responseEntries.data) responseEntries.message = messages.DATA_NOT_FOUND;
        }
    } catch (error) {
        responseEntries.error = true;
...
async function updateStaffOnDuty(req, res) {
    ...
    try {
        ...
        if (validationResponse != true) {
            ...
            responseEntries.data = await staffOnDutyServices.updateStaffOnDuty(req.params.staffOnDutyId, req.body);
            if (!responseEntries.data) responseEntries.message = messages.DATA_NOT_FOUND;
        }
    } catch (error) {
        responseEntries.error = true;
